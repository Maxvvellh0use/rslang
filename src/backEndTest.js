...
import UserStatistics from './data/UserStatistics';
import {UserStatisticsModel, DayStatisticsModel}  from './models/UserStatisticsModel';

export const backEndTest = async () => {


  let newUser = new UserModel({
        email: 'User999@herokuapp.com',
        name: 'Tom',
  ...
  try {
    ...
    console.log('login new user: ', authUser);
  } catch (error) {
    console.log(error);
    return;
  }
  ...
  let statistics;
  try {
    statistics = await UserStatistics.getUserStatistics(authUser);
  ...
  try {
    const userStatistics = new UserStatisticsModel({learnedWords: 5});
    console.log(userStatistics);
    statistics = await UserStatistics.updateUserStatistics({authUser, userStatistics});
    console.log('Updated user statistics: ', statistics);
  } catch (error) {
    console.log(error);    
  }
